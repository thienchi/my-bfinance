...
<ASP:PANEL ID="pnlList" runat="server" CssClass="bannerbar">
    ...
    <ASP:REPEATER ID="rptList" runat="server">
        <ITEMTEMPLATE>
            <div class="newsf">
                <div class="blockimg"><a href="<%#Eval("eUrl") %>"><%#Eval("eFilepreview") %></a></div>
                <div class="cname"><%#Eval("Cname") %></div>
                <div class="lname"><a href="<%#Eval("eUrl") %>"><%#Eval("Name")%></a> <%#Eval("eIconex") %></div>
                <div class="intr"><%#Eval("Introduce")%> <a class="more" href="<%#Eval("eUrl") %>?pag=<%=PageIndex %>">...chi tiết</a></div>
                <div class="newsfupnext" id="upnext<%#Container.ItemIndex %>"><a href="<%#Eval("eUrl") %>"><%#Eval("Name").ToString().Length > 60 ? Eval("Name").ToString().Substring(0, 60) + "..." : Eval("Name").ToString()%></a></div>
            </div>
        </ITEMTEMPLATE>
    </ASP:REPEATER>
        </div>
