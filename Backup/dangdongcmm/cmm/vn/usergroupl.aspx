<%@ PAGE Language="C#" MasterPageFile="MasterDefault.Master" AutoEventWireup="true" CodeBehind="usergroupl.aspx.cs" Inherits="dangdongcmm.cmm.usergroupl" %>
            ...
            <ASP:GRIDVIEW ID="grdView" runat="server" DataKeyNames="Id" CssClass="lgridview" 
                ...
                <COLUMNS>
                    ...
                    <ASP:TEMPLATEFIELD itemstyle-cssclass="colcheck">
                        ...
                        <ITEMTEMPLATE>
                            <input type="checkbox" id="chkcheck<%# Eval("Id")%>" name="chkcheck" value="<%# Eval("Id")%>" onclick="chkAll('chkcheckall', this);" <%#Eval("Id").ToString() == "1" ? "disabled" : ""%> />
                        </ITEMTEMPLATE>
                    </ASP:TEMPLATEFIELD>
                    <ASP:TEMPLATEFIELD headertext="Tên nhóm" sortexpression="Name">
                        <ITEMTEMPLATE>
                            <a><%#Eval("Name") %></a>
                        </ITEMTEMPLATE>
                    </ASP:TEMPLATEFIELD>
                    <ASP:TEMPLATEFIELD headertext="Người dùng" sortexpression="Depth" itemstyle-cssclass="colpis">
                        <ITEMTEMPLATE>
                            <a href='userl.aspx?pid=<%#Eval("Id") %>'><%#Eval("Pis").ToString() == "0" ? "" : ("Người dùng (" + Eval("ePis") + ")")%></a>
                        </ITEMTEMPLATE>
                    </ASP:TEMPLATEFIELD>
                    ...
                    <ASP:TEMPLATEFIELD itemstyle-cssclass="colaction">
                        <ITEMTEMPLATE>
                            <a class="command" href="javascript:CC_gotoUrl('usergroupu.aspx?pid=<%#Eval("Pid") %>&iid=<%#Eval("Id") %>');" <%#Eval("Id").ToString() == "1" ? "style='visibility:hidden'" : "style='visibility:visible'"%>>Sửa</a>
                            &nbsp;|&nbsp;
